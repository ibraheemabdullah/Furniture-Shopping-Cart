...
<div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="date" rows="6" name="dateTime" ng-model="eventCtrl.event.dateTime" class="form-control"  id="inputSuccess">
        <span ng-show="eventForm.dateTime.$error.required" class="msg-val">Date is required.</span>
      </div>
</div>  
    
   <div ng-if="eventCtrl.flag != 'edit'">
		       <button type="button" class="w3-btn w3-teal" ng-click="eventCtrl.addEvent()"> Create Event</button> 
		       <button type="button" class="w3-btn w3-teal" ng-click="eventCtrl.reset()">Reset</button>
		       
		    </div>
		     <span ng-if="eventCtrl.flag=='created'" class="msg-success">Event successfully added.</span>
		    
	 <div ng-if="eventCtrl.flag == 'edit'">
		       <button  type="button" class="w3-btn w3-teal" ng-click="eventCtrl.updateEventDetail()">Update Event</button> 	
			   <button type="button" class="w3-btn w3-teal" ng-click="eventCtrl.cancelUpdate()">Cancel</button>				   
		    </div>
 ...
 <table  class="w3-table w3-striped w3-bordered w3-border">
	      ...
	      <tr ng-repeat="row in eventCtrl.events">
	         ...
	         <td>
		    <button type="button"  class="w3-btn w3-teal" ng-click="eventCtrl.deleteEvent(row.id)">Delete</button>
		    <button type="button"  class="w3-btn w3-teal" ng-click="eventCtrl.editEvent(row.id)" >Edit</button>
